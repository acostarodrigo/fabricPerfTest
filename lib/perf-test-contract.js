...
const collection = 'collection_Broker1MSP';
class PerfTestContract extends Contract {    
    ...
    async createPerfTest(ctx, value) {
        ...
        await ctx.stub.setEvent("createPerfTest", Buffer.from(JSON.stringify(value)));
    ...
    async updatePerfTest(ctx, perfTestId, newValue) {
        ...
        // we trigger the event
        await ctx.stub.setEvent("updatePerfTest", Buffer.from(JSON.stringify(asset)));
    ...
    async createPrivatePerfTest(ctx) {
        ...
        const asset = { value };
        const buffer = Buffer.from(JSON.stringify(asset));
        await ctx.stub.putPrivateData(collection, perfTestId, buffer);
    ...
    async deletePrivatePerfTest(ctx, perfTestId) {
        ...
        await ctx.stub.deletePrivateData(collection, perfTestId);
    }
